...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            # num_tokens_with_spec可以理解成当前这个请求一共有多少token需要做计算
            # prompt_token_ids：表示这个请求的prompt
            # output_token_ids：表示这个请求当前已经生成的response。举例来说，刚做完prefill的请求，
            # 会产生第一个output_token装入output_token_ids。后续的decode过程中，每产出一个token，都会装入这里，这个列表是动态变化的。
            # spec_token_ids：推测解码策略相关的token_ids，不是本文讲述的重点，我们可以将其长度视为0
            # num_new_tokens: 它即表示当前这个请求有多少token还没做计算
            ...
            if num_new_tokens == 0:
                # 请求无法调度，原因如下：
                # 1. 没有新的token可供调度。这可能发生在以下情况：
                # (1) PP>1，且我们已经调度了所有prompt tokens，但尚未完成。
                # (2) 异步调度，且请求已达到其 max_total_tokens 或 max_model_len。
                # 2. 编码器预算已耗尽。
                # 3. 编码器缓存已耗尽。
                # 注意：此处，通过使用 `continue` 而不是 `break`，
                # 我们没有严格遵循 FCFS 调度策略，并允许调度优先级较低的请求。
                req_index += 1
                continue
