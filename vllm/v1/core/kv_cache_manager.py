...
class KVCacheManager:
    ...
    def allocate_slots(
        ...
        """
        # < computed >：本次调度前，就已经分配给该请求的blocks,<computed>: 0。waiting队列中所有的请求都是如此
        # < new computed >：本次调度时，该请求在prefix caching下新命中的blocks。即有几个缓存block可用
        # 对于该请求来说，< computed >和< new computed >都是现成的blocks，前者已经是它的，后者它可以复用。
        # 所以这些blocks都不需要从free_block_queue队列中再去取。
