...
from vllm_module.logger import (_DATE_FORMAT, _FORMAT, _configure_vllm_root_logger,
                                enable_trace_function_call, init_logger)
from vllm_module.logging import NewLineFormatter
...
def test_default_vllm_root_logger_configuration():
    ...
    logger = logging.getLogger("vllm_module")
...
@patch("vllm_module.logger.VLLM_CONFIGURE_LOGGING", 1)
@patch("vllm_module.logger.VLLM_LOGGING_CONFIG_PATH", None)
def test_descendent_loggers_depend_on_and_propagate_logs_to_root_logger():
    """This test presumes that VLLM_CONFIGURE_LOGGING (default: True) and
    VLLM_LOGGING_CONFIG_PATH (default: None) are not configured and default
    behavior is activated."""
    root_logger = logging.getLogger("vllm_module")
    root_handler = root_logger.handlers[0]

    unique_name = f"vllm_module.{uuid4()}"
...
@patch("vllm_module.logger.VLLM_CONFIGURE_LOGGING", 0)
@patch("vllm_module.logger.VLLM_LOGGING_CONFIG_PATH", None)
...
@patch("vllm_module.logger.VLLM_CONFIGURE_LOGGING", 1)
@patch(
    "vllm_module.logger.VLLM_LOGGING_CONFIG_PATH",
...
def test_an_error_is_raised_when_custom_logging_config_is_invalid_json():
    ...
    with NamedTemporaryFile(encoding="utf-8", mode="w") as logging_config_file:
        ...
        with patch("vllm_module.logger.VLLM_LOGGING_CONFIG_PATH",
...
def test_custom_logging_config_is_parsed_and_used_when_provided():
    ...
    valid_logging_config = {
        "loggers": {
            "vllm_module.test_logger.logger": {
                "handlers": [],
                "propagate": False,
            }
        },
...
def test_custom_logging_config_causes_an_error_if_configure_logging_is_off():
    ...
    with NamedTemporaryFile(encoding="utf-8", mode="w") as logging_config_file:
        ...
        root_logger = logging.getLogger("vllm_module")
        other_logger_name = f"vllm_module.test_logger.{uuid4()}"
