...
from vllm2.config import LoRAConfig
from vllm2.lora.fully_sharded_layers import (
...
# yapf: disable
from vllm2.lora.layers import (BaseLayerWithLoRA, ColumnParallelLinearWithLoRA,
...
# yapf: enable
from vllm2.lora.models import (LongContextLoRAContext, LoRALayerWeights,
                              PackedLoRALayerWeights)
from vllm2.lora.punica_wrapper import get_punica_wrapper
...
def check_punica_wrapper(punica_wrapper) -> bool:
    if current_platform.is_cuda_alike():
        from vllm2.lora.punica_wrapper.punica_gpu import PunicaWrapperGPU

        return type(punica_wrapper) is PunicaWrapperGPU
    elif current_platform.is_cpu():
        from vllm2.lora.punica_wrapper.punica_cpu import PunicaWrapperCPU
...
def test_vocab_parallel_embedding_indices(tp_size, seed):
    ...
    for tp_rank in range(tp_size):
        with patch(
                "vllm2.model_executor.layers.vocab_parallel_embedding.get_tensor_model_parallel_rank",
                return_value=tp_rank
        ), patch(
                "vllm2.model_executor.layers.vocab_parallel_embedding.get_tensor_model_parallel_world_size",
