...
def do_sample(llm: vllm_module.LLM, lora_path: str, lora_id: int) -> List[str]:
    ...
    sampling_params = vllm_module.SamplingParams(temperature=0,
                                                 max_tokens=256,
                                                 stop=["[/assistant]"])
...
def test_llama_lora_warmup(sql_lora_files):
    ...
    def get_num_gpu_blocks_lora():
        llm = vllm_module.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16)
        num_gpu_blocks_lora_warmup = llm.llm_engine.cache_config.num_gpu_blocks
        return num_gpu_blocks_lora_warmup

    @ray.remote(num_gpus=1)
    def get_num_gpu_blocks_no_lora():
        llm = vllm_module.LLM(MODEL_PATH, max_num_seqs=16)
