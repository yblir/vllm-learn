...
from vllm_module.utils import Counter, random_uuid
...
class AsyncLLM:
    ...
    Note: Current LLM class in vllm_module don't support async mode, for test purpose,
    we implement async one in here. Maybe we could move to
    vllm_module/entrypoints/llm.py in future.

    Below AsyncLLM is directly borrow from vllm_module/entrypoints/llm.py with changes
