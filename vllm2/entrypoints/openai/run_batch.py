...
from vllm2.engine.arg_utils import AsyncEngineArgs, nullable_str
from vllm2.engine.async_llm_engine import AsyncLLMEngine
from vllm2.entrypoints.logger import RequestLogger
from vllm2.entrypoints.openai.protocol import (BatchRequestInput,
                                               BatchRequestOutput,
                                               BatchResponseData,
...
from vllm2.utils import FlexibleArgumentParser, random_uuid
...
async def run_request(chat_serving: OpenAIServingChat,
    ...
    if isinstance(chat_response, ChatCompletionResponse):
        batch_output = BatchRequestOutput(
            id=f"vllm2-{random_uuid()}",
            custom_id=request.custom_id,
            response=BatchResponseData(
                body=chat_response, request_id=f"vllm2-batch-{random_uuid()}"),
            error=None,
        )
        ...
        batch_output = BatchRequestOutput(
            ...
            response=BatchResponseData(
                ...
                request_id=f"vllm2-batch-{random_uuid()}"),
            error=chat_response,
        )
    else:
