...
DEFAULT_LOGGING_CONFIG = {
    "formatters": {
        "vllm2": {
            "class": "vllm2.logging.NewLineFormatter",
...
def init_logger(name: str) -> Logger:
    ...
    retrieved in such a way that we can be sure the root vllm2 logger has
...
def _trace_calls(log_path, root_dir, frame, event, arg=None):
    if event in ['call', 'return']:
        ...
        if not filename.startswith(root_dir):
            # only log the functions in the vllm2 root_dir
            return
...
def enable_trace_function_call(log_file_path: str,
    ...
    if root_dir is None:
        # by default, this is the vllm2 root directory
