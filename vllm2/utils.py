...
from vllm2.logger import enable_trace_function_call, init_logger
...
def is_openvino() -> bool:
    from importlib.metadata import PackageNotFoundError, version
    try:
        return "openvino" in version("vllm2")
    except PackageNotFoundError:
        return False
...
def is_xpu() -> bool:
    ...
    # todo
    # is_xpu_flag = "xpu" in version("vllm2")
    is_xpu_flag=False
    # vllm2 is not build with xpu
    if not is_xpu_flag:
        return False
    try:
...
def get_vllm_instance_id() -> str:
    ...
    """
    return envs.VLLM_INSTANCE_ID or f"vllm2-instance-{random_uuid()}"
...
def enable_trace_function_call_for_thread() -> None:
    ...
    if envs.VLLM_TRACE_FUNCTION:
        ...
        log_path = os.path.join(tmp_dir, "vllm2", get_vllm_instance_id(),
                                filename)
