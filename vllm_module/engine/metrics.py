...
from vllm_module.executor.ray_utils import ray
...
if TYPE_CHECKING:
    from vllm_module.spec_decode.metrics import SpecDecodeWorkerMetrics
...
class Metrics:
    ...
    def __init__(self, labelnames: List[str], max_model_len: int):
        ...
        self.gauge_scheduler_running = self._gauge_cls(
            name="vllm_module:num_requests_running",
            documentation="Number of requests currently running on GPU.",
            labelnames=labelnames)
        self.gauge_scheduler_waiting = self._gauge_cls(
            name="vllm_module:num_requests_waiting",
            documentation="Number of requests waiting to be processed.",
            labelnames=labelnames)
        self.gauge_scheduler_swapped = self._gauge_cls(
            name="vllm_module:num_requests_swapped",
            documentation="Number of requests swapped to CPU.",
            labelnames=labelnames)
        #   KV Cache Usage in %
        self.gauge_gpu_cache_usage = self._gauge_cls(
            name="vllm_module:gpu_cache_usage_perc",
            documentation="GPU KV-cache usage. 1 means 100 percent usage.",
            labelnames=labelnames)
        self.gauge_cpu_cache_usage = self._gauge_cls(
            name="vllm_module:cpu_cache_usage_perc",
            documentation="CPU KV-cache usage. 1 means 100 percent usage.",
            labelnames=labelnames)

        # Iteration stats
        self.counter_num_preemption = self._counter_cls(
            name="vllm_module:num_preemptions_total",
            documentation="Cumulative number of preemption from the engine.",
            labelnames=labelnames)
        self.counter_prompt_tokens = self._counter_cls(
            name="vllm_module:prompt_tokens_total",
            documentation="Number of prefill tokens processed.",
            labelnames=labelnames)
        self.counter_generation_tokens = self._counter_cls(
            name="vllm_module:generation_tokens_total",
            documentation="Number of generation tokens processed.",
            labelnames=labelnames)
        self.histogram_time_to_first_token = self._histogram_cls(
            name="vllm_module:time_to_first_token_seconds",
        ...
        self.histogram_time_per_output_token = self._histogram_cls(
            name="vllm_module:time_per_output_token_seconds",
        ...
        self.histogram_e2e_time_request = self._histogram_cls(
            name="vllm_module:e2e_request_latency_seconds",
            documentation="Histogram of end to end request latency in seconds.",
            labelnames=labelnames,
            buckets=[1.0, 2.5, 5.0, 10.0, 15.0, 20.0, 30.0, 40.0, 50.0, 60.0])
        #   Metadata
        self.histogram_num_prompt_tokens_request = self._histogram_cls(
            name="vllm_module:request_prompt_tokens",
            documentation="Number of prefill tokens processed.",
            labelnames=labelnames,
    ...
    def _create_info_cache_config(self) -> None:
        ...
        self.info_cache_config = prometheus_client.Info(
            name='vllm_module:cache_config',
            documentation='information of cache_config')

    def _unregister_vllm_metrics(self) -> None:
        for collector in list(prometheus_client.REGISTRY._collector_to_names):
            if hasattr(collector, "_name") and "vllm_module" in collector._name:
