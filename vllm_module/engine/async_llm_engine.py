...
from vllm_module.config import (DecodingConfig, EngineConfig, LoRAConfig, ModelConfig,
                                ParallelConfig, SchedulerConfig)
from vllm_module.core.scheduler import SchedulerOutputs
from vllm_module.engine.arg_utils import AsyncEngineArgs
from vllm_module.engine.async_timeout import asyncio_timeout
from vllm_module.engine.llm_engine import LLMEngine
from vllm_module.engine.metrics import StatLoggerBase
from vllm_module.executor.executor_base import ExecutorAsyncBase
from vllm_module.executor.ray_utils import initialize_ray_cluster, ray
from vllm_module.inputs import LLMInputs, PromptInputs
...
from vllm_module.sequence import ExecuteModelRequest, SamplerOutput
from vllm_module.usage.usage_lib import UsageContext
...
class AsyncLLMEngine:
    ...
    def _get_executor_cls(
        ...
        if isinstance(distributed_executor_backend, type):
            ...
            from vllm_module.executor.neuron_executor import NeuronExecutorAsync
            executor_class = NeuronExecutorAsync
        elif engine_config.device_config.device_type == "tpu":
            if distributed_executor_backend == "ray":
                initialize_ray_cluster(engine_config.parallel_config)
                from vllm_module.executor.ray_tpu_executor import RayTPUExecutorAsync
                executor_class = RayTPUExecutorAsync
            else:
                assert distributed_executor_backend is None
                from vllm_module.executor.tpu_executor import TPUExecutorAsync
                executor_class = TPUExecutorAsync
        elif engine_config.device_config.device_type == "cpu":
            from vllm_module.executor.cpu_executor import CPUExecutorAsync
            executor_class = CPUExecutorAsync
        elif engine_config.device_config.device_type == "openvino":
            assert distributed_executor_backend is None, (
                "Distributed execution is not supported with "
                "the OpenVINO backend.")
            from vllm_module.executor.openvino_executor import OpenVINOExecutorAsync
            executor_class = OpenVINOExecutorAsync
        elif engine_config.device_config.device_type == "xpu":
            if distributed_executor_backend is None:
                from vllm_module.executor.xpu_executor import XPUExecutorAsync
                executor_class = XPUExecutorAsync
            elif distributed_executor_backend == "ray":
                initialize_ray_cluster(engine_config.parallel_config)
                from vllm_module.executor.ray_xpu_executor import RayXPUExecutorAsync
            ...
            initialize_ray_cluster(engine_config.parallel_config)
            from vllm_module.executor.ray_gpu_executor import RayGPUExecutorAsync
            ...
            from vllm_module.executor.gpu_executor import GPUExecutorAsync
    ...
    def from_engine_args(
        ...
        if engine_args.engine_use_ray:
            from vllm_module.executor import ray_utils
    ...
    async def generate(
        ...
        Details:
            - If the engine is not running, start the background loop,
              ...
              :meth:`~vllm_module.engine.async_llm_engine.AsyncLLMEngine.engine_step`
