...
import vllm_module.envs as envs
from vllm_module.config import (CacheConfig, DecodingConfig, DeviceConfig,
                                EngineConfig, LoadConfig, LoRAConfig, ModelConfig,
                                MultiModalConfig, ObservabilityConfig, ParallelConfig,
                                PromptAdapterConfig, SchedulerConfig,
                                SpeculativeConfig)
from vllm_module.core.scheduler import (ScheduledSequenceGroup, Scheduler,
                                        SchedulerOutputs)
from vllm_module.engine.arg_utils import EngineArgs
from vllm_module.engine.metrics import (LoggingStatLogger, PrometheusStatLogger,
                                        StatLoggerBase, Stats)
from vllm_module.engine.output_processor.interfaces import (
    SequenceGroupOutputProcessor)
from vllm_module.engine.output_processor.stop_checker import StopChecker
from vllm_module.engine.output_processor.util import create_output_by_sequence_group
from vllm_module.executor.executor_base import ExecutorBase
from vllm_module.executor.ray_utils import initialize_ray_cluster
from vllm_module.inputs import INPUT_REGISTRY, LLMInputs, PromptInputs
...
from vllm_module.sequence import (EmbeddingSequenceGroupOutput, ExecuteModelRequest,
                                  PoolerOutput, SamplerOutput, Sequence,
                                  SequenceGroup, SequenceGroupMetadata,
                                  SequenceStatus)
from vllm_module.tracing import (SpanAttributes, SpanKind, extract_trace_context,
                                 init_tracer)
from vllm_module.transformers_utils.config import try_get_generation_config
from vllm_module.transformers_utils.detokenizer import Detokenizer
from vllm_module.transformers_utils.tokenizer_group import (
    AnyTokenizer, BaseTokenizerGroup, init_tokenizer_from_configs)
from vllm_module.usage.usage_lib import (UsageContext, is_usage_stats_enabled,
                                         usage_message)
from vllm_module.utils import Counter
from vllm_module.version import __version__ as VLLM_VERSION
...
class LLMEngine:
    ...
    The :class:`~vllm_module.LLM` class wraps this class for offline batched inference
    and the :class:`AsyncLLMEngine` class wraps this class for online serving.

    The config arguments are derived from :class:`~vllm_module.EngineArgs`. (See
    :ref:`engine_args`)
    ...
    def __init__(
        ...
        if is_usage_stats_enabled():
            from vllm_module.model_executor.model_loader import (
    ...
    def _get_executor_cls(cls, engine_config: EngineConfig) -> Type[ExecutorBase]:
        ...
        if isinstance(distributed_executor_backend, type):
            ...
            from vllm_module.executor.neuron_executor import NeuronExecutor
            executor_class = NeuronExecutor
        elif engine_config.device_config.device_type == "tpu":
            if distributed_executor_backend == "ray":
                initialize_ray_cluster(engine_config.parallel_config)
                from vllm_module.executor.ray_tpu_executor import RayTPUExecutor
                executor_class = RayTPUExecutor
            else:
                assert distributed_executor_backend is None
                from vllm_module.executor.tpu_executor import TPUExecutor
                executor_class = TPUExecutor
        elif engine_config.device_config.device_type == "cpu":
            from vllm_module.executor.cpu_executor import CPUExecutor
            executor_class = CPUExecutor
        elif engine_config.device_config.device_type == "openvino":
            from vllm_module.executor.openvino_executor import OpenVINOExecutor
            executor_class = OpenVINOExecutor
        elif engine_config.device_config.device_type == "xpu":
            if distributed_executor_backend == "ray":
                initialize_ray_cluster(engine_config.parallel_config)
                from vllm_module.executor.ray_xpu_executor import RayXPUExecutor
                executor_class = RayXPUExecutor
            else:
                from vllm_module.executor.xpu_executor import XPUExecutor
                executor_class = XPUExecutor
        elif distributed_executor_backend == "ray":
            initialize_ray_cluster(engine_config.parallel_config)
            from vllm_module.executor.ray_gpu_executor import RayGPUExecutor
            executor_class = RayGPUExecutor
        elif distributed_executor_backend == "mp":
            from vllm_module.executor.multiproc_gpu_executor import (
            ...
            from vllm_module.executor.gpu_executor import GPUExecutor
    ...
    def add_request(
        ...
        Args:
            ...
            inputs: The inputs to the LLM. See
                :class:`~vllm_module.inputs.PromptInputs`
                for more details about the format of each input.
            params: Parameters for sampling or pooling.
                :class:`~vllm_module.SamplingParams` for text generation.
                :class:`~vllm_module.PoolingParams` for pooling.
        ...
        Details:
            ...
            - Create `best_of` number of :class:`~vllm_module.Sequence` objects.
            - Create a :class:`~vllm_module.SequenceGroup` object
              from the list of :class:`~vllm_module.Sequence`.
            - Add the :class:`~vllm_module.SequenceGroup` object to the scheduler.
    ...
    def abort_request(self, request_id: Union[str, Iterable[str]]) -> None:
        ...
        Details:
            - Refer to the
              :meth:`~vllm_module.core.scheduler.Scheduler.abort_seq_group`
              from class :class:`~vllm_module.core.scheduler.Scheduler`.
