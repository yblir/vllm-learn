...
from vllm_module.engine.arg_utils import AsyncEngineArgs, nullable_str
from vllm_module.engine.async_llm_engine import AsyncLLMEngine
from vllm_module.entrypoints.logger import RequestLogger
from vllm_module.entrypoints.openai.protocol import (BatchRequestInput,
                                                     BatchRequestOutput,
                                                     BatchResponseData,
...
from vllm_module.utils import FlexibleArgumentParser, random_uuid
...
async def run_request(chat_serving: OpenAIServingChat,
    ...
    if isinstance(chat_response, ChatCompletionResponse):
        batch_output = BatchRequestOutput(
            id=f"vllm_module-{random_uuid()}",
            custom_id=request.custom_id,
            response=BatchResponseData(
                body=chat_response, request_id=f"vllm_module-batch-{random_uuid()}"),
            error=None,
        )
        ...
        batch_output = BatchRequestOutput(
            ...
            response=BatchResponseData(
                ...
                request_id=f"vllm_module-batch-{random_uuid()}"),
            error=chat_response,
        )
    else:
